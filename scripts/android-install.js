...
module.exports = function (context) {
    ...
    // if (plugins.length > 0 && plugins.indexOf('com.wosai.qpay') === -1) {
    //     return ;
    // }
    ...
    var targetDir  = path.join(projectRoot, "platforms", "android", "src", packageName.replace(/\./g, path.sep), "qpay");
        targetFiles = ["CallbackActivity.java"];
    ...
    if (['after_plugin_add', 'after_plugin_install'].indexOf(context.hook) === -1) {
        ...
        targetFiles.forEach(function (f) {
            fs.readFile(path.join(context.opts.plugin.dir, 'src', 'android', f), {encoding: 'utf-8'}, function (err, data) {
                ...
                data = data.replace(/^package __PACKAGE_NAME__;/m, 'package ' + packageName + '.qpay;');
